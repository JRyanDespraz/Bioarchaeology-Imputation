...
#define the imputation methods for each column based on data type
imputation_methods <- c(rep("lasso.select.logreg", 2), rep("polyreg", 5), rep("polr", 6), rep("lasso.select.norm", 28))

#set "m" equal to the number of imputations desired 
#set "meth" equal to the desired imputation methods
mice_imputed <- mice(dataset_with_missingness, m=10, maxit=50, meth=imputation_methods, seed=99) 
